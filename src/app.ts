...
import { pool } from './db';
import { CreateLinkRequestBody, CreateLinkResponseBody, ErrorResponseBody, LinkRow } from './types';
...
export function buildApp(): Express {
  ...
  app.post(
    '/links',
    async (
      req: Request<{}, CreateLinkResponseBody | ErrorResponseBody, CreateLinkRequestBody>,
  ...
  app.get(
    '/:shortCode',
    async (req: Request<{ shortCode: string }>, res: Response<ErrorResponseBody | void>) => {
      const { shortCode } = req.params;

      try {
        const result = await pool.query<Pick<LinkRow, 'original_url' | 'expires_at'>>(
